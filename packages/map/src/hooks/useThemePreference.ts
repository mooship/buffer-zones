 ...
 * @remarks Interim copy pending `@stratum/react` (Task 3).
 */
export type ThemePreference = "system" | "light" | "dark";

/** `localStorage` key under which the explicit theme preference is persisted. */
export const THEME_STORAGE_KEY = "buffer-zones-theme";

/** `theme-color` meta content for each explicit (non-`"system"`) theme. */
...
/**
 * Sets the theme preference, persists it to `localStorage`, applies the
 * `data-theme` attribute and `theme-color` meta tag, and notifies subscribers.
...
/**
 * Subscribes to the current theme preference, updating on every
 * `setThemePreference` call.
 * @returns The current `ThemePreference`.
 */
