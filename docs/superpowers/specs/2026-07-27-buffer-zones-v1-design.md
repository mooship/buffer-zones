...
Ship a real, working version of Buffer Zones — a map that visualizes apartheid-era spatial planning's effect on commute times in a South African city — scoped to what's actually buildable in this environment and session, using real data rather than placeholders.

## 2. Deltas from SPEC.md (and why)

| SPEC.md says | v1 does instead | Why |
|---|---|---|
| 4 metros, 7 transit operators | Cape Town only; other operators scaffolded but empty | One metro done for real beats four done thinly. Adapter/registry extensibility (SPEC §11) is still validated by scaffolding the rest. |
...
| Shapefile→GeoJSON via `ogr2ogr` (GDAL) | Prefer Adrian Frith's already-GeoJSON Cape Town sub-place data; fall back to npm `shapefile` + `@turf/turf` only if needed | No system GDAL installed; installing it isn't necessary since the pipeline is Node/TS already (SPEC §3) and a pure-JS path avoids a new system dependency entirely. |
...
| Unemployment choropleth (SPEC §7) | Included only if a real, cleanly-attributable Stats SA ward-level CT download is found; otherwise ships as a clearly-labeled empty/"data not yet available" state | Non-goal is fabricating numbers to fill a UI slot (SPEC §8: "not aiming for real-time accuracy" — but that's about accuracy, not fabrication). |
...
│   │   ├── src/
│   │   ├── public/data/  # townships.v1.geojson, myciti.v1.geojson, prasa.v1.geojson
│   │   ├── vite.config.ts
│   │   └── package.json
...
└── data-pipeline/         # offline scripts, not an npm workspace, committed
    ├── src/
    │   ├── adapters/      # one per source (frith-boundaries, myciti, prasa-osm, + stubs for gautrain/rea-vaya/metrobus/a-re-yeng/durban-transport)
...
1. **Boundaries** — fetch Adrian Frith's Cape Town sub-place GeoJSON (or Stats SA shapefile + npm `shapefile`/`@turf/turf` conversion as fallback). Normalize to `TownshipProperties` base shape (id, name, population if available).
2. **Job center** — single hardcoded reference point: Cape Town CBD lat/lon, in a `constants/jobCenters.ts`.
3. **Routing** — for each township centroid, batch-query `router.project-osrm.org/table/v1/driving/...` for drive time to the CBD point. Respect public-server rate limits: small batches, delay between requests, retry on 429.
4. **Transit** —
   - MyCiTi: fetch real GeoJSON from City of Cape Town's open data portal export.
   - PRASA: query Overpass API for `railway=rail`, `operator=PRASA` within a Cape Town bounding box.
   - Adapters normalize each into the common `TransitLayer` shape (SPEC §10 Adapter pattern).
5. **Unemployment** — attempt a real Stats SA ward-level Cape Town download; if unavailable in a usable form, the layer definition still exists in the registry but its `dataSource` resolves to an empty dataset and the UI shows "data not yet available for this area" rather than any number.
6. **Join & export** — merge commute time + transit proximity onto township features; write `townships.v1.geojson`, `myciti.v1.geojson`, `prasa.v1.geojson` to `packages/web/public/data`. Stub adapters for Gautrain/Rea Vaya/Metrobus/A Re Yeng/Durban Transport exist with typed interfaces and a registry entry each, but no data file — documented in `data-pipeline/README.md` as the mechanical next step (validates SPEC §11's extensibility claim without requiring those cities' data now).

## 5. Frontend (v1)

Unchanged from SPEC.md §4, minus anything AI/chat-related:
- Leaflet choropleth colored by `commute_minutes` to Cape Town CBD
- Toggleable MyCiTi and PRASA overlays
...
- Joburg, Tshwane, Durban data and their transit operators (Gautrain, Rea Vaya, Metrobus, A Re Yeng, Durban Transport) — adapters/types exist, data doesn't yet
