# Mogale City Included-Area Classification
...
- all Mogale City Census 2011 sub-places used for municipality-wide comparison; and
- a current inclusion set of township and historically marginalised settlement areas highlighted with dissolved outlines.

Stats SA does not publish a `township` boundary type in this dataset. The inclusion set is therefore a project classification, not an official or exhaustive list of Mogale City townships.
...
Mogale City's municipality code (`MN_CODE`) in the Stats SA Census 2011 sub-place shapefile is 763 (`MN_MDB_C` GT481).
