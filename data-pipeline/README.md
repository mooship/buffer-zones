...
Gautrain rail, Gautrain Bus, and PRASA/Metrorail are Gauteng-wide networks fetched for every metro; A Re Yeng (Tshwane only) and Rea Vaya (Johannesburg only) are each a single city's own BRT operator. All five are real, live layers. Currently stubbed (no typed adapter, no verified public source found yet): MyCiTi (Cape Town), Metrobus (Johannesburg's own conventional bus operator — a real Overpass adapter was tried on 2026-07-29 but OSM coverage turned out to be a single verified stop with no route geometry, too sparse to ship), Durban Transport — see design doc §8.
